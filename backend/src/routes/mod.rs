...
pub fn build_router(state: AppState) -> Router {
    ...
    let public = Router::new()
        ...
        .route(
            "/groups/:id/members",
            get(groups::list_group_members_handler),
        )
        .route(
            "/groups/:id/payout-schedule",
            get(groups::get_payout_schedule_handler),
        )
    ...
    let protected = Router::new()
        ...
        .route(
            "/groups/:id/join",
            axum::routing::post(groups::join_group_handler),
        )
        .route(
            "/groups/:id/activate",
            axum::routing::patch(groups::activate_group_handler),
        )
        .route(
            "/groups/:id/advance-payout",
            axum::routing::post(groups::advance_payout_handler),
        )
        .route(
            "/contributions",
            axum::routing::post(contributions::record_contribution_handler),
        )
        .route(
            "/groups/:group_id/me",
            get(members::get_my_membership_handler),
        )
        .route(
            "/groups/:group_id/members/:member_id",
            axum::routing::delete(members::remove_member_handler),
        )
        .route("/auth/me", get(auth::me_handler))
        .route(
            "/auth/profile",
            axum::routing::patch(auth::update_profile_handler),
        )
